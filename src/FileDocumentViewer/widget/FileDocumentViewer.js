        ...
        _updateRendering: function (callback) {
            ...
            if (this._contextObj && this._contextObj.get("HasContents")) {
                if (this._usePDFjs /* && this._contextObj.get("Name").indexOf(".pdf") !== -1*/ ) {
                    ...
                    if (this._contextObj.get("Name").indexOf(".stl") !== -1) {
                        var stlviewerr = require.toUrl("FileDocumentViewer/lib/stlview/index.html").split("?")[0],
                        encoded = stlviewerr + "?file=" + encodeURIComponent(this._getFileUrl());
                        domAttr.set(this.iframeNode, "src", encoded);
                    } else if (this._contextObj.get("Name").indexOf(".obj") !== -1) {
                        var stlviewerr = require.toUrl("FileDocumentViewer/lib/stlview/index.html").split("?")[0],
                        encoded = stlviewerr + "?objfile=" + encodeURIComponent(this._getFileUrl());
                        domAttr.set(this.iframeNode, "src", encoded);
                    }  
                    else {
